/**
 * =========================================================
 * FE (面板固定效应) 回归模块 - 经典大刊 6 列版
 * BY张张 | 支持面板双向固定、稳健与聚类无缝衔接
 * =========================================================
 */

const generateFEStataCode = (vars, config, fileInfo, processingSteps) => {
    const { y, x, c } = vars;
    ...
    code += `* 步骤3: 渐进式固定效应回归分析\n`;
    let models = [];
    let absorbBase = [`\`panel_id'`].filter(v => v).join(" ");
    let absorbTWFE = [`\`panel_id'`, stYear].filter(v => v).join(" ");

    code += `reghdfe ${stY} ${stX}, absorb(${absorbBase})\nestadd local cluster "无"\nestadd local fe_entity "Yes"\nestadd local fe_time "No"\nest store m1\n`;
    models.push('m1');

    if (stCluster) {
        code += `reghdfe ${stY} ${stX}, absorb(${absorbBase}) vce(cluster \`cluster_id')\nestadd local cluster "${clusterVar}"\nestadd local fe_entity "Yes"\nestadd local fe_time "No"\nest store m2\n`;
        models.push('m2');
    } else {
        code += `reghdfe ${stY} ${stX}, absorb(${absorbBase}) vce(robust)\nestadd local cluster "稳健"\nestadd local fe_entity "Yes"\nestadd local fe_time "No"\nest store m2\n`;
        models.push('m2');
    }

    if (safeC.length > 0) {
        code += `reghdfe ${stY} ${stX} ${stC}, absorb(${absorbBase})\nestadd local cluster "无"\nestadd local fe_entity "Yes"\nestadd local fe_time "No"\nest store m3\n`;
        models.push('m3');

        if (stCluster) {
            code += `reghdfe ${stY} ${stX} ${stC}, absorb(${absorbBase}) vce(cluster \`cluster_id')\nestadd local cluster "${clusterVar}"\nestadd local fe_entity "Yes"\nestadd local fe_time "No"\nest store m4\n`;
            models.push('m4');
        } else {
            code += `reghdfe ${stY} ${stX} ${stC}, absorb(${absorbBase}) vce(robust)\nestadd local cluster "稳健"\nestadd local fe_entity "Yes"\nestadd local fe_time "No"\nest store m4\n`;
            models.push('m4');
        }

        if (stYear) {
            code += `* 引入高维固定效应 (双向固定)\n`;
            code += `reghdfe ${stY} ${stX} ${stC}, absorb(${absorbTWFE})\nestadd local cluster "无"\nestadd local fe_entity "Yes"\nestadd local fe_time "Yes"\nest store m5\n`;
            models.push('m5');

            if (stCluster) {
                code += `reghdfe ${stY} ${stX} ${stC}, absorb(${absorbTWFE}) vce(cluster \`cluster_id')\nestadd local cluster "${clusterVar}"\nestadd local fe_entity "Yes"\nestadd local fe_time "Yes"\nest store m6\n`;
                models.push('m6');
            } else {
                code += `reghdfe ${stY} ${stX} ${stC}, absorb(${absorbTWFE}) vce(robust)\nestadd local cluster "稳健"\nestadd local fe_entity "Yes"\nestadd local fe_time "Yes"\nest store m6\n`;
                models.push('m6');
            }
        }
    }
    ...
    code += `\n* 步骤4: 导出结果\n`;
    code += `esttab ${models.join(' ')} using "results_fe.rtf", replace ///\n`;
    ...
    code += `    title("表2: 面板个体固定效应 (FE) 回归结果")\n`;
...
ModelRegistry.register({
        ...
        template: `
            <div style="width: 100%;">
                <table class="academic-table">
                    <caption>表1. FE 固定效应阶梯回归矩阵</caption>
                    <thead>
                        <tr class="top-line">
                    ...
                    <tbody>
                        ...
                        <tr><td class="var-name">_cons</td><td>-</td><td v-if="results.m2">-</td><td v-if="results.m3">-</td><td v-if="results.m4">-</td><td v-if="results.m5">-</td><td v-if="results.m6">-</td></tr>
                    </tbody>
                    <tfoot>
                        ...
                        <tr><td class="text-left">个体效应</td><td>Yes</td><td v-if="results.m2">Yes</td><td v-if="results.m3">Yes</td><td v-if="results.m4">Yes</td><td v-if="results.m5">Yes</td><td v-if="results.m6">Yes</td></tr>
                ...
                <div style="font-size: 13px; color: #64748b; margin: 8px auto 0; max-width: 950px; text-align: left; padding-left: 10px;">注：***、**、*分别表示在1%、5%、10%的水平上显著；括号内为普通标准误或聚类稳健标准误。</div>
            </div>
        `
    },
    ...
    run: (vars, data, config) => {
        ...
        let m1 = runMiniReghdfe(y, safeX, data, [config.entityVar], 'ordinary', config.clusterVar);
        let m2 = runMiniReghdfe(y, safeX, data, [config.entityVar], config.clusterVar ? 'cluster' : 'robust', config.clusterVar);
        let m3 = null, m4 = null, m5 = null, m6 = null;

        if (safeC.length > 0) {
            m3 = runMiniReghdfe(y, allX, data, [config.entityVar], 'ordinary', config.clusterVar);
            m4 = runMiniReghdfe(y, allX, data, [config.entityVar], config.clusterVar ? 'cluster' : 'robust', config.clusterVar);
            if (config.yearVar) {
                m5 = runMiniReghdfe(y, allX, data, [config.entityVar, config.yearVar], 'ordinary', config.clusterVar);
                m6 = runMiniReghdfe(y, allX, data, [config.entityVar, config.yearVar], config.clusterVar ? 'cluster' : 'robust', config.clusterVar);
            }
        } else if (config.yearVar) {
            m5 = runMiniReghdfe(y, safeX, data, [config.entityVar, config.yearVar], 'ordinary', config.clusterVar);
            m6 = runMiniReghdfe(y, safeX, data, [config.entityVar, config.yearVar], config.clusterVar ? 'cluster' : 'robust', config.clusterVar);
        }
        return { m1, m2, m3, m4, m5, m6 };
    },
    generateCode: (vars, config, fileInfo, processingSteps) => generateFEStataCode(vars, config, fileInfo, processingSteps)
});